...
@section('header_content')
    <div class="d-flex justify-content-between mg-b-5">
        <div class="d-flex">
            ...
            <div class="ml-3">
                <h4 class="tx-gray-800">Supplier</h4>
                <h6 class="ml-2">List Supplier</h6>
            </div>
        </div>
...
@section('body_content')
    ...
    {{-- Modal Add --}}
    <div id="modal-supplier_add" class="modal fade" data-value=''>
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content tx-size-sm">
                <div class="modal-header pd-x-20">
                    <h5></h5>
                    <h6 class="tx-20 mg-b-0 tx-inverse tx-bold"> <i class="fa fa-plus mg-r-10"></i> Add Supplier</h6>
    ...
    {{-- Modal Edit --}}
    <div id="modal-supplier_edit" class="modal fade" data-value=''>
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content tx-size-sm">
                <div class="modal-header pd-x-20">
                    <h5></h5>
                    <h6 class="tx-20 mg-b-0 tx-inverse tx-bold">Edit Suppler</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                ...
                <div class="modal-body-padding">
                    ...
                    <form id="form_edit_item">
                        ...
                        <input type="text" name="supplier_id" id="edit-supplier_id" hidden>
    ...
    {{-- Modal Delete --}}
    <div id="modal-supplier_delete" class="modal fade" data-value=''>
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content tx-size-sm">
                ...
                <div class="modal-body-padding">
                    ...
                    <div class="d-flex justify-content-end">
                        <input type="text" name="supplier_id" id="delete-supplier_id" hidden>
                        <button name="button" data-dismiss="modal" aria-label="Close"
                            class='rounded-xl btn btn-dark'>Cancel</button>
...
@section('javascript')
    <script>
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.result === 'error_validate') {
                        ...
                        table.ajax.reload()
                        $('#modal-supplier_edit').modal("hide");
                        amaran_success(res.message)
                    } else {
                        amaran_error(res.message)
                    }
        ...
        $("#delete_item").click(() => {
            const data = {
                supplier_id: $("#delete-supplier_id").val()
            }
            $.ajax({
                url: '{{ route('supplier_delete') }}',
                method: 'POST',
                data,
                datatype: "json",
                headers: {
                ...
                success: function(res) {
                    if (res.result === 'SUCCESS') {
                        table.ajax.reload()
                        $('#modal-supplier_delete').modal("hide");
                        amaran_success(res.message)
                    } else {
                        amaran_error(res.message)
                    }
        ...
        $('#table-data tbody').on('click', '#delete-modal-show', function() {
            ...
            $("#delete-supplier_id").val(supplier_id);
            $(".name-delete").text(name);
            $('#modal-supplier_delete').modal("show");
            $.LoadingOverlay('hide');
        });
        ...
        $('#table-data tbody').on('click', '#edit-modal-show', function() {
            ...
            $.LoadingOverlay("hide");
            $("#modal-supplier_edit").modal("show");
        })

        // Modal Add
        $(".modal-add").click(() => {
            $.LoadingOverlay("show");
            $('#modal-supplier_add').modal("show");
            $.LoadingOverlay('hide');
        })
    </script>
@endsection
